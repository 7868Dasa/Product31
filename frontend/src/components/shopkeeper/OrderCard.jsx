  ...
  return (
    <article className="rounded-xl2 border-2 border-sand bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-semibold text-ink-soft">{t('sk.order')}</div>
          <div className="font-mono text-2xl font-extrabold tracking-widest text-primary">
            {order.order_code}
          </div>
        </div>
        {isPending ? (
          ...
          <span className="inline-flex items-center rounded-full bg-primary-tint px-3 py-1 text-sm font-bold text-primary-dark">
      ...
      <div className="mt-4 flex gap-2">
        {order.status === 'PENDING_ACCEPTANCE' && (
          <>
            <button
              ...
              className="flex flex-1 items-center justify-center gap-1.5 rounded-full border-2 border-stop/40 py-3 font-semibold text-stop active:bg-stop/10"
            >
              <X size={18} /> {t('sk.reject')}
            </button>
            <button
              onClick={() => onAccept(order.id)}
              className="flex flex-[2] items-center justify-center gap-1.5 rounded-full bg-primary py-3 font-semibold text-white active:bg-primary-dark"
            >
        ...
        {order.status === 'ACCEPTED' && (
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-full bg-primary py-3 font-semibold text-white active:bg-primary-dark"
          >
        ...
        {order.status === 'READY_FOR_PICKUP' && (
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-full bg-go py-3 font-semibold text-white active:opacity-90"
          >
